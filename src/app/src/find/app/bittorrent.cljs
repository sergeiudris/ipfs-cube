...
(defn decode-values
  [valuesB]
  (for [i (range 0 (.-length valuesB) 6)]
    (let [idB (.slice valuesB i (+ i 6))]
      {:address (str (aget valuesB (+ i 0)) "."
                     (aget valuesB (+ i 1)) "."
                     (aget valuesB (+ i 2)) "."
                     (aget valuesB (+ i 3)))
       :port (.readUInt16BE valuesB (+ i 4))})))
...
(defn send-get-peers
  [socket rifno node-idB infohashB]
  (send-krpc
   socket
   (clj->js
...
(defn discover-torrent
  [{:keys [routing-table socket msg|mult node-idB infohashB]}]
  (go
    (let [txn-idsA (atom #{})
          ...
          release (fn []
                    ...
                    (close! nodes|)
                    (close! seeders|)
                    (doseq @cancel-channelsA
                      (close! cancel|)))]

      (doseq [node closest-nodes]
        (put! nodes| node))
    ...
    (let [stateA (atom
        ...
        (go
          (loop []
            (when-let [{:keys [infohashB rinfo]} (<! infohash|tap)]
              #_(put! infohash| {:infohash (.toString infohashB "hex")})
              (recur)))))
      ...
      (go
        (loop []
          ...
          (doseq [[id {:keys [node timestamp]}] (:routing-table-find-noded @stateA)]
            (when (> (- (js/Date.now) timestamp) (* 5 60 1000))
