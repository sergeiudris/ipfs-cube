(ns ipfs-shipyard.find.bittorrent-dht-crawl.core
  (:require
   ...
   [ipfs-shipyard.find.bittorrent-dht-crawl.dht]
   [ipfs-shipyard.find.bittorrent-dht-crawl.find-nodes]
   [ipfs-shipyard.find.bittorrent-dht-crawl.metadata]
   [ipfs-shipyard.find.bittorrent-dht-crawl.sybil]
   [ipfs-shipyard.find.bittorrent-dht-crawl.sample-infohashes]))
...
(defn start
  [{:as opts
    :keys [data-dir]}]
  (go
    (let [state-filepath (fs.runtime.core/path-join data-dir "ipfs-shipyard.find.bittorrent-dht-crawl.core.json")
          stateA (atom
                  (merge
      ...
      (ipfs-shipyard.find.bittorrent-dht-crawl.dht/start-routing-table
       (merge ctx {:routing-table-max-size 128}))


      (ipfs-shipyard.find.bittorrent-dht-crawl.dht/start-dht-keyspace
       (merge ctx {:routing-table-max-size 128}))
      ...
      (let [stop| (chan 1)]
        (swap! procsA conj stop|)
        (ipfs-shipyard.find.bittorrent-dht-crawl.find-nodes/start-bootstrap-query
         (merge ctx {:stop| stop|})))

      ; periodicaly ask nodes for new nodes
      (let [stop| (chan 1)]
        (swap! procsA conj stop|)
        (ipfs-shipyard.find.bittorrent-dht-crawl.find-nodes/start-dht-query
         (merge ctx {:stop| stop|})))

      ; start sybil
      #_(let [stop| (chan 1)]
          (swap! procsA conj stop|)
          (ipfs-shipyard.find.bittorrent-dht-crawl.sybil/start
           {:stateA stateA
            :nodes-bootstrap nodes-bootstrap
            :sybils| sybils|
      ...
      (ipfs-shipyard.find.bittorrent-dht-crawl.sample-infohashes/start-sampling
       ctx)

      ; discovery
      (ipfs-shipyard.find.bittorrent-dht-crawl.metadata/start-discovery
       (merge ctx
...
(defn process-print-info
  ...
  (let [started-at (now)
        filepath (fs.runtime.core/path-join data-dir "ipfs-shipyard.find.crawl-log.edn")
          ...
          ([_]
           ...
           (let [state @stateA
                 info [[:count @countA]
                       ...
                       [:sockets @ipfs-shipyard.find.bittorrent-dht-crawl.metadata/count-socketsA]
...
(comment
  ...
  (require '[ipfs-shipyard.find.bittorrent-dht-crawl.core :as bittorrent-dht-crawl.core] :reload-all)
  ...
  -ro '{:host "0.0.0.0"
        :port 8899}' \
  --repl-env node --compile ipfs-shipyard.find.bittorrent-dht-crawl.core --repl
  ...
  (require
   ...
   :reload #_:reload-all)
    ...
    (bittorrent-dht-crawl.core/start
